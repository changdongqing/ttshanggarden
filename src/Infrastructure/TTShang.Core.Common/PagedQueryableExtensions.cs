...
namespace TTShang.Core.Common
{
    ...
    public static class PagedQueryableExtensions
    {
        ...
        /// <returns></returns>
        public static PageList<TEntity> ToPage<TEntity>(this IQueryable<TEntity> entities, int pageIndex = 1, int pageSize = 20)
        ...
        /// <returns></returns>
        public static async Task<PageList<TEntity>> ToPageAsync<TEntity>(this IQueryable<TEntity> entities, int pageIndex = 1, int pageSize = 20, CancellationToken cancellationToken = default)
            where TEntity : class, new()
        {
        ...
        public static IQueryable<T> OrderConditions<T>(this IQueryable<T> query, List<ListSortDirection> orderConditions)
        {
            ...
            return query.OrderConditions(orderConditions.ToArray());
        }
        /// <summary>
        /// 多字段排序
        /// </summary>
        ...
        public static IQueryable<T> OrderConditions<T>(this IQueryable<T> query, ListSortDirection[] orderConditions)
        {
            ...
            for (var i = 0; i < orderConditions.Length; i++)
            {
                ...
                if (property == null)
                {
                    throw Oops.BahLocalFrom<SharedLocalResource>(ExceptionCode.Field_In_Type_Not_Found, orderinfo.FieldName, t.Name);
                }
                //创建一个访问属性的表达式
