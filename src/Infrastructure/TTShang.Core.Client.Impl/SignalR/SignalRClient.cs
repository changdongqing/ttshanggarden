...
namespace TTShang.Core.Client.Impl.SignalR
{
    ...
    public class SignalRClient : ISignalRClient
    {
        ...
        /// 使用默认重试策略
        /// </summary>
        private bool _useDefaultRetryPolicy = true;
        /// <summary>
        /// 默认重试策略 延迟时间
        /// </summary>
        private TimeSpan[]? _defaultRetryPolicyReconnectDelays;
        /// <summary>
        /// 自定义重试策略
        /// </summary>
        private IRetryPolicy? _customRetryPolicy;
        /// <summary>
        /// 是否自动重连
        /// </summary>
        private bool _automaticReconnect = false;
        /// <summary>
        /// 连接
        /// </summary>
        ...
        /// <param name="jsonSerializerOptions">json序列化设置</param>
        public SignalRClient(string clientName, string url, IClientLogger clientLogger, Func<Task<string?>> accessTokenProvider, ILocalizationLocalizer localizer, JsonSerializerOptions? jsonSerializerOptions = null)
        {
        ...
        {
            string msg = localizer.Combination(nameof(SharedLocalResource.Connection), arg == null ? nameof(SharedLocalResource.Close) : nameof(SharedLocalResource.Break));
            clientLogger.Warn($"{ClientName} {msg}", ex: arg, sendNotify: arg != null);
            if (Closed != null)
            {
                //回调
        ...
        {
            clientLogger.Warn($"{ClientName} {localizer[nameof(SharedLocalResource.Reconnection)]}", ex: arg);
        ...
        {
            clientLogger.Warn($"{ClientName} {localizer[nameof(SharedLocalResource.Reconnected)]} ![{(arg == null ? "" : arg)}]");
            if (Reconnected != null)
            {
        ...
        /// 开启自动重连-默认策略
        /// </summary>
        /// <param name="defaultRetryPolicyReconnectDelays">默认重试策略，每次延迟时间</param>
        /// <returns></returns>
        public ISignalRClient AutomaticReconnectDefaultRetryPolicy(TimeSpan[]? defaultRetryPolicyReconnectDelays = null)
        {
            _automaticReconnect = true;
            _useDefaultRetryPolicy = true;
            _defaultRetryPolicyReconnectDelays = defaultRetryPolicyReconnectDelays;
            return this;
        }
        /// <summary>
        /// 开启自动重连
        ...
        /// <returns></returns>
        public ISignalRClient AutomaticReconnectCustomRetryPolicy(long maxRetryNumber, double retryIntervalSeconds)
        {
            _automaticReconnect = true;
            _useDefaultRetryPolicy = false;
            _customRetryPolicy = new CustomRetryPolicy(maxRetryNumber, retryIntervalSeconds);
            return this;
        }
        /// <summary>
        /// 创建连接并开启
        ...
        public Task Connection()
        {
            ...
            {
                Uri uri = new Uri(url);
                clientLogger.Info($"{ClientName} {localizer.Combination(nameof(SharedLocalResource.Begin), nameof(SharedLocalResource.Connection))},url={uri.AbsoluteUri}");
                ...
                IHubConnectionBuilder builder = new HubConnectionBuilder()
                            ...
                            }).AddJsonProtocol(jsonOpt =>
                            {
                                jsonOpt.PayloadSerializerOptions = jsonSerializerOptions;
                            });
                //配置重连
                if (_automaticReconnect)
                {
                    if (!_useDefaultRetryPolicy && _customRetryPolicy != null)
                    {
                        builder.WithAutomaticReconnect(_customRetryPolicy);
                    }
                    else
                    {

                        if (_defaultRetryPolicyReconnectDelays == null)
                        {
                            builder.WithAutomaticReconnect();
                        }
                        else
                        {
                            builder.WithAutomaticReconnect(_defaultRetryPolicyReconnectDelays);
                        }
                    }
            ...
            {
                clientLogger.Error($"{ClientName} {localizer.Combination(nameof(SharedLocalResource.Connection), nameof(SharedLocalResource.Error))}", ex: ex);
            }
            return Task.CompletedTask;
        }
        ...
        public ISignalRClient On<TResut>(string methodName, Action<TResut?> resutHandler)
        {
            if (connection == null)
            {
                ...
                {
                    resutHandler(data == null ? default : (TResut)data);
                };
                //暂存
        ...
        public ISignalRClient On<TResut>(string methodName, Func<TResut?, Task> resutHandler)
        {
            if (connection == null)
            {
                ...
                {
                    return resutHandler(data == null ? default : (TResut)data);
                };
                //暂存
                messageCallBacks.Add(new MessageCallBackInfo(methodName, typeof(TResut), callBack));
            }
        ...
        private class MessageCallBackInfo
        {
            ...
            {
                MethodName = methodName;
                ArgumentTypes = new Type[] { argType };
                CallBack = delegate (object?[] objs)
                {
                    callBack(objs[0]);
                    return Task.CompletedTask;
