...
namespace Gardener.Core.Api.Impl.UserCenter.Services
{
    ...
    public class RoleService : ServiceBase<Role, RoleDto, int, GardenerMultiTenantDbContextLocator>, IRoleService
    {
        ...
        public RoleService(IRepository<Role, GardenerMultiTenantDbContextLocator> roleRepository, IRepository<RoleResource, GardenerMultiTenantDbContextLocator> roleResourceRepository, IAuthService authorizationService, ITenantConfigService tenantConfigService) : base(roleRepository)
        {
        ...
        public override async Task<RoleDto> Insert(RoleDto input)
        {
            ...
            {
                SystemTenantConfigDto? systemTenantConfig = await tenantConfigService.GetTenantConfigByConfigKey(identity.TenantId.Value, "TenantMaxRoleNumber");
                if (systemTenantConfig != null && !string.IsNullOrEmpty(systemTenantConfig.ConfigValue))
                {
                    int count = _repository.Count(false);
                    if (count >= int.Parse(systemTenantConfig.ConfigValue))
                    {
                        throw Oops.BahLocalFrom<UserCenterResource>(ExceptionCode.Maximum_Number_Of_Roles_Has_Been_Reached);
                    }
                }
            }
        ...
        public async Task<string> GetRoleResourceSeedData()
        {
            ...
            return SeedDataGenerateHelper.Generate(roleResources, typeof(RoleResource).Name);
        }
    }
}
