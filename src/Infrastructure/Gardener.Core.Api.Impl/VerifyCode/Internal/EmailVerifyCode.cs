...
using Gardener.Core.Api.Impl.VerifyCode.Internal.Settings;
...
namespace Gardener.Core.Api.Impl.VerifyCode.Internal
{
    ...
    /// 邮件验证码服务
    /// </summary>
    internal class EmailVerifyCode : IVerifyCode
    {
        ...
        {
            await store.Remove(VerifyCodeTypeEnum.Email, key);
            return true;
        }
        ...
        private async Task<VerifyCodeOutput> Send(string toEmail, CharacterCodeCreateParam? param = null)
        {
            ...
            await emailService.Send(new SendEmailInputDto
            {
                Data = new { Code = code },
            ...
            await store.Add(VerifyCodeTypeEnum.Email, key, code, settings.CodeExpire);
        ...
        public async Task<bool> Verify(string key, string code)
        {
            ...
            string? dbCode = await store.GetCode(VerifyCodeTypeEnum.Email, key);
