...
namespace TTShang.Core.Client.Components.PageBaseClass
{
    ...
    public abstract class TableBase<TDto, TKey, TLocalResource, TSelfOperationDialogInput, TSelfOperationDialogOutput> : ReuseTabsPageAndFormBase<TSelfOperationDialogInput, TSelfOperationDialogOutput> where TDto : class, new()
    {
        ...
        /// </summary>
        [Inject]
        protected IClientNotifier ClientNotifier { get; set; } = null!;

        /// <summary>
        /// 客户端日志
        /// </summary>
        [Inject]
        protected IClientLogger ClientLogger { get; set; } = null!;
        ...
        /// <returns></returns>
        protected Task OpenOperationDialogAsync<TOperationDialog, TDialogInput, TDialogOutput>(string title, TDialogInput input, Func<TDialogOutput?, Task>? onClose = null, OperationDialogSettings? operationDialogSettings = null, string? width = null) where TOperationDialog : FeedbackComponent<TDialogInput, TDialogOutput>
        {
        ...
        {
            return _table?.GetPageRequest() ?? new PageRequest() { PageIndex = 1, PageSize = ClientConstant.PageSize };
        }
        ...
        protected virtual async Task OnClickExport()
        {
            ...
            {
                ClientLogger.Error(Localizer.Combination(nameof(SharedLocalResource.Export), nameof(SharedLocalResource.Fail)), ex: ex);
            }
