...
namespace Gardener.Core.Client.Components.PageBaseClass
{
        ...
        protected async Task OnClickDelete(TDto dto, bool trueDelete)
        {
            if (await ConfirmService.YesNoDelete() == ConfirmResult.Yes)
            {
                ...
                if (result)
                {
                    ...
                    //if (_datas != null && pKey != null && DeleteTreeNode(pKey.Value, GetKey(dto), _datas))
                    //{
        ...
        protected bool DeleteTreeNode(TKey pId, TKey id, IEnumerable<TDto> items)
        {
            foreach (TDto dto in items)
            {
                ...
                if (key.Equals(pId))
                {
                    ...
                    SetChildren(dto, children.Where(x => !GetKey(x).Equals(id)).ToArray());
                    return true;
                }
                else
                {
    ...
    public abstract class TreeTableBase<TDto, TKey, TOperationDialog, TLocalResource> : TreeTableBase<TDto, TKey, TOperationDialog, OperationDialogInput<TKey>, OperationDialogOutput<TKey>, TLocalResource>
        ...
        where TDto : class, new() where TKey : struct
    {
        /// <summary>
        /// 根据<TDto>获取查看时传入抽屉的数据项<TEditOption>
        /// </summary>
        ...
        protected override async Task OnEditFinish(TDto dto, OperationDialogOutput<TKey>? dialogOutput)
        {
            if (dialogOutput != null && dialogOutput.Succeeded)
            {
                ...
                _datas.ForEach(x =>
                {
                    ...
                    if (pKey != null)
                    {
                        var p = GetNodeFromTree(pKey.Value, x);
                        if (p != null)
                        {
    ...
    /// <typeparam name="TKey"></typeparam>
    /// <typeparam name="TOperationDialog"></typeparam>
    public abstract class TreeTableBase<TDto, TKey, TOperationDialog> : TreeTableBase<TDto, TKey, TOperationDialog, SharedLocalResource> where TOperationDialog : OperationDialogBase<OperationDialogInput<TKey>, OperationDialogOutput<TKey>, SharedLocalResource> where TDto : class, new() where TKey : struct
    { }
}
