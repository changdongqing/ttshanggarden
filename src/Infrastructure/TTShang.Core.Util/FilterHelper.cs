...
namespace TTShang.Core.Util
{
    ...
    public static class FilterHelper
    {
        ...
        private static LambdaExpression? GetPropertyLambdaExpression(ParameterExpression param, FilterRule rule)
        {
            ...
            PropertyInfo? property = properties.Where(x => x.Name.Equals(propertyName)).OrderBy(x =>
            {
                return type.Equals(x.DeclaringType) ? 0 : 1;
            }).FirstOrDefault();
            if (property == null)
            {
                //属性不存在，不生成表达式
            ...
            if (rule.Operate.Equals(FilterOperate.In))
            {
                ...
                if (rule.Value is JsonElement)
                {
                    ...
                    if (values.ValueKind.Equals(JsonValueKind.Array))
                    {
                        ...
                        {
                            throw new Exception($"{ExceptionCode.Query_Value_Type_No_Find_Converter}  {conversionType.Name}");
                        }
