...
namespace TTShang.Core.Api.Impl.Authorization.Internal
{
    ...
    /// 身份权限服务
    /// </summary>
    internal class IdentityPermissionService : IIdentityPermissionService
    {
        ...
        public Task<bool> IsSuperAdministrator(Identity? identity)
        {
            ...
            {
                //身份信息缺失
                throw Oops.BahLocalFrom<SharedLocalResource>(ExceptionCode.Unauthorized);
            }
            if (IdentityType.User.Equals(identity.IdentityType))
            {
        ...
        private async Task<bool> CurrentUserHaveFunction(int userId, string functionKey)
        {
            ...
            {
                //租户资源判断
                var result = await _tenantResourceRepository.AsQueryable(false)
                    .Where(x => !x.Tenant.IsLocked && !x.Tenant.IsDeleted && x.IsLocked.Equals(false) && x.IsDeleted.Equals(false) && x.TenantId.Equals(user.TenantId) && functionResourceIds.Contains(x.ResourceId)).AnyAsync();
                if (!result)
                {
                    return false;
                }
            ...
            //查找是否包含指定角色和资源编号
            return await _roleResourceRepository.AsQueryable(false).Where(x => !x.IsDeleted && !x.IsLocked && roleIds.Contains(x.RoleId) && functionResourceIds.Contains(x.ResourceId)).Select(x => x.ResourceId).AnyAsync();
            //一步到位，减少请求次数
        ...
        private async Task<bool> CurrentUserHaveResource(int userId, string resourceKey)
        {
            ...
            var resource = await _resourceRepository.AsQueryable(false).Where(x => !x.IsDeleted && !x.IsLocked && x.Key.Equals(resourceKey)).FirstOrDefaultAsync();
            if (resource == null)
            {
                return false;
            }
            ...
            {
                //租户资源判断
                var result = await _tenantResourceRepository.AsQueryable(false).Where(x => !x.Tenant.IsLocked && !x.Tenant.IsDeleted && x.IsLocked.Equals(false) && x.IsDeleted.Equals(false) && x.TenantId.Equals(user.TenantId) && x.ResourceId.Equals(resource.Id)).AnyAsync();
                if (!result)
                {
                    return false;
                }
