...
namespace TTShang.Core.Api.Impl.EntityFramwork.Internal
{
    /// <summary>
    /// 全局实体基础信息处理
    /// </summary>
    internal static class GlobalEntityEntryHandle
    {
        /// <summary>
        /// 处理
        /// </summary>
        ...
        public static void Handle(IEnumerable<EntityEntry>? entityEntries, Identity? identity, bool handleTenant = false)
        {
            ...
            entityEntries = entityEntries
                  ...
                  && u.Entity.GetType() != typeof(AuditFunction)
        ...
        private static void CheckIsCurrentTenantData(EntityEntry entity, Identity identity)
        {
            ...
            if (propertyValues != null)
            {
                ...
                if (propertyValues.TryGetValue(nameof(IModelTenantId.TenantId), out tenantId))
                {
                    ...
                    {
                        throw Oops.BahLocalFrom<SharedLocalResource>(ExceptionCode.No_Permission_Modify_The_Data);
                    }
                }
            }
        }
