...
namespace TTShang.Core.Api.Impl.NotificationSystem.Internal
{
    /// <summary>
    /// 系统通知集线器
    /// </summary>
    ...
    public class SystemNotificationHub : Hub
    {
        ...
        public Task Send(NotificationData data)
        {
            ...
            data.Ip = Context.GetHttpContext().GetRemoteIpAddressToIPv4(true);
            data.FromConnectionId = Context.ConnectionId;
        ...
        public override async Task OnConnectedAsync()
        {
            ...
            string ip = Context.GetHttpContext().GetRemoteIpAddressToIPv4(true);
            await systemNotificationService.SetUserOnline(identity, Context.ConnectionId, ip,Context.ConnectionAborted);
            ...
            foreach (IEnumerable<string> groups in groupNameGroups)
            {
                ...
                {
                    tasks1.Add(Groups.AddToGroupAsync(Context.ConnectionId, group));
                }
            }
            if (tasks1.Any())
            {
        ...
        public override async Task OnDisconnectedAsync(Exception? exception)
        {
            ...
            await systemNotificationService.SetUserOffline(identity, Context.ConnectionId);
            //分组器
            IEnumerable<ISystemNotificationHubGrouper> groupers = App.GetServices<ISystemNotificationHubGrouper>();
            List<IEnumerable<string>> groupNameGroups = new();
            foreach (var grouper in groupers)
            ...
            foreach (IEnumerable<string> groups in groupNameGroups)
            {
                ...
                foreach (var group in groups)
                {
                    tasks1.Add(Groups.RemoveFromGroupAsync(Context.ConnectionId, group));
                }
            }
            if (tasks1.Any())
            {
            ...
            {
                Identity = identity,
                Ip = Context.GetHttpContext().GetRemoteIpAddressToIPv4(true),
