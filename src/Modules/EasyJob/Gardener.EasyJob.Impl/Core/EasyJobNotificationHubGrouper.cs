...
namespace Gardener.EasyJob.Impl.Core
{
    ...
    /// 将需要监听用户进行分组
    /// </summary>
    public class EasyJobNotificationHubGrouper : ISystemNotificationHubGrouper
    {
        ...
        public async Task<IEnumerable<string>> GetGroupName(Identity identity)
        {
            ...
            bool exits = await userConfigRepository.AnyAsync(x => x.IdentityId.Equals(identity.Id) && x.IdentityType.Equals(identity.IdentityType) && x.EnableRealTimeMonitor==true && x.IsDeleted == false && x.IsLocked == false);
            if (exits)
            {
