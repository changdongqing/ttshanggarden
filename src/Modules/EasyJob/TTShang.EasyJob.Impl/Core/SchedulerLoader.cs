...
namespace TTShang.EasyJob.Impl.Core
{
    ...
    public class SchedulerLoader
    {
        ...
        public JobBuilder CreateJobBuilder(SysJobDetail jobDetail)
        {
            ...
            switch (jobDetail.CreateType)
            {
                case JobCreateType.Script when string.IsNullOrEmpty(jobDetail.ScriptCode):
                    throw Oops.Bah(Lo.GetValue<ValidateErrorMessagesResource>(nameof(ValidateErrorMessagesResource.RequiredValidationError)), Lo.GetValue<EasyJobLocalResource>(nameof(EasyJobLocalResource.ScriptCode)));
                case JobCreateType.Script:
                    {
                        jobType = _dynamicJobCompiler.BuildJob(jobDetail.ScriptCode);
                        if (jobType == null)
                        {
                            throw Oops.BahLocalFrom<SharedLocalResource>(EasyJobExceptionCode.Script_Code_Compile_Fail);
                        }
                        if (jobType.GetCustomAttributes(typeof(JobDetailAttribute), false).FirstOrDefault() is not JobDetailAttribute jobDetailAttribute)
                            throw Oops.BahLocalFrom<SharedLocalResource>(EasyJobExceptionCode.Script_Code_JobDetail_Not_Find);
                        if (jobDetailAttribute.JobId != jobDetail.JobId)
                            throw Oops.BahLocalFrom<SharedLocalResource>(EasyJobExceptionCode.Script_Code_JobId_Inconsistency);
        ...
        public JobBuilder CreateOrUpdateJobBuilder(SysJobDetail newJooDetail, SysJobDetail oldJobDetail)
        {
            ...
            //if (scheduler == null)
            //{
            //    throw Oops.Bah(ExceptionCode.Scheduler_Not_Find);
            //}
