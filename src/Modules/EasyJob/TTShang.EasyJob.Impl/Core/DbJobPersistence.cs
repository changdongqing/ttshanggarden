...
using TTShang.EasyJob.Dtos.Notification;
...
namespace TTShang.EasyJob.Impl.Core
{
    ...
    /// 作业持久化（数据库）
    /// </summary>
    internal class DbJobPersistence : IJobPersistence
    {
        ...
        public Task OnChangedAsync(PersistenceContext context)
        {
            ...
            var jobDetailRepository = scope.ServiceProvider.GetRequiredService<IRepository<SysJobDetail, GardenerIgnoreAuditDbContextLocator>>();
        ...
        {
            using var scope = _serviceScopeFactory.CreateScope();
            var jobTriggerRepository = scope.ServiceProvider.GetRequiredService<IRepository<SysJobTrigger, GardenerIgnoreAuditDbContextLocator>>();
        ...
        public async Task<IEnumerable<SchedulerBuilder>> PreloadAsync(CancellationToken stoppingToken)
        {
            ...
            // 若数据库不存在任何作业，则直接返回
            if (!await _jobRepository.AnyAsync(u => true)) return allJobs;
            ...
            foreach (var schedulerBuilder in allJobs)
            {
                ...
                // 加载数据库数据
                var dbDetail = await _jobRepository.AsQueryable(false).FirstOrDefaultAsync(u => u.JobId == jobBuilder.JobId);
            ...
            var dbJobs = await _jobRepository.AsQueryable(false).Where(x => x.IsLocked == false && x.IsDeleted == false).ToListAsync();
            ...
            {
                if (allJobs.Any(x => dbJob.JobId.Equals(x.GetJobBuilder().JobId))) { continue; }
                List<TriggerBuilder> triggers = new List<TriggerBuilder>();
                var dbTriggers = await _triggerRepository.Where(u => u.JobId == dbJob.JobId).ToListAsync();
