...
namespace TTShang.WoChat.Impl.Core.Subscribes
{
    /// <summary>
    /// 订阅东方财富资讯
    /// </summary>
    public class DongFangCaiFuNewsEventSubscriber : IEventSubscriber, ISingleton
    {
        ...
        [EventSubscribe(nameof(EventGroup.SystemNotify) + "Gardener.WoChat.Dtos." + nameof(DongFangCaiFuNewsEvent))]
        public async Task News(EventHandlerExecutingContext context)
        {
            ...
            foreach (var im in imSessionIds)
            {
                ...
                }, new Identity(userId.ToString(),IdentityType.User)));
            }
