...
using TTShang.WoChat.Impl.Entities;

namespace TTShang.WoChat.Impl.Services
{
    /// <summary>
    /// Im聊天服务
    /// </summary>
    [ApiDescriptionSettings("WoChat", Module = "wo-chat")]
    public class WoChatImService : IWoChatImService, IDynamicApiController
    {
        ...
        public WoChatImService(IAuthService authorizationService, IRepository<ImSession, GardenerMultiTenantDbContextLocator> imSessionRepository, IRepository<ImSessionMessage, GardenerMultiTenantDbContextLocator> imSessionMessageRepository, IRepository<ImUserSession, GardenerMultiTenantDbContextLocator> imUserSessionRepository, ISystemNotificationService systemNotificationService, IUserService userService)
        {
        ...
        private string GetSignature(IEnumerable<int> userIds)
        {
            ...
            return MD5Helper.Encrypt(userIdStrs);
        }
        /// <summary>
        /// 添加会话
        /// </summary>
        ...
        public async Task<Guid?> AddMyImSession(ImSessionAddInput input)
        {
            ...
            foreach (var userId in userIds)
            {
                ...
                await systemNotificationService.UserGroupAdd(WoChatUtil.GetImGroupName(imSession.Id),identity.IdentityType, userId.ToString());
            }
            return imSession.Id;
        }
        /// <summary>
            ...
            else if (session.SessionType.Equals(ImSessionType.Group))
            {
                if (userId.ToString().Equals(session.CreateBy))
                {
                    ...
                    userSessions.ForEach(async x =>
                    {
                        ...
                        tasks.Add(systemNotificationService.UserGroupRemove(WoChatUtil.GetImGroupName(imSessionId),IdentityType.User,x.UserId.ToString()));
                    });
        ...
        public async Task<bool> SendMessage(ImSessionMessageDto message, Identity? identity)
        {
            ...
            {
                //禁言中
                throw Oops.BahLocalFrom<SharedLocalResource>(Enums.ExceptionCode.SessionDisableSendMessage);
            }
